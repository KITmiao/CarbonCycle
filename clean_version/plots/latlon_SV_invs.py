...
def meteo_spatial(fig, ax, value, obj, period, cmap, vmin, vmax):
    ...
    if topo:
        elev_spatial(ax)
    ax.set_title(f'seasonal varibility GOSAT/{obj.upper()}')
...
def plts(value, obj, cmap, range):
    ...
    return ax1, cbar
print(__name__)
if __name__.split('.')[-1] == 'latlon_SV_invs':
    from settings import gosat_var
    ...
    if gosat_var == 'mean':
        variable = (data.gosat['nbp'].spatial['RemoTeC'].sel(time=slice(spatial_period['start'], spatial_period['end']))
        +   data.gosat['nbp'].spatial['ACOS'].sel(time=slice(spatial_period['start'], spatial_period['end'])))/2
    else:
        variable = data.gosat['nbp'].spatial[gosat_var].sel(time=slice(spatial_period['start'], spatial_period['end']))
    std       = variable.groupby("time.year").std(dim='time')
    sv       = std.mean(dim='year')
    sv       = sa.zscore(sv)
    plot1, cbar1 = plts(sv, gosat_var, 'RdYlBu_r', [-3,3])
    cbar1.set_label('CO$_2$ seasonal variability')
