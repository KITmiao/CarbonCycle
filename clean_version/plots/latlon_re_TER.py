...
print(__name__)
if __name__.split('.')[-1] == 'latlon_re_TER':
    ...
    nbp    = (data.gosat['nbp'].spatial['RemoTeC'] + data.gosat['nbp'].spatial['ACOS'])/2
    nirv   = data.est['nirvgpp'].spatial
    nirv   = get_mean(-nirv*0.001+nbp, spatial_period)
    sif    = data.est['sifgpp'].spatial
    sif    = get_mean(-sif*(0.01/30)+nbp, spatial_period)
    ...
    if ano:
        ...
        nirv        = data.est['nirvgpp'].spatial
        nirv_clim   = get_mean(-nirv*0.001+nbp, all_periods)
        nirv        = get_mean(-nirv * 0.001+nbp, spatial_period)
        sif         = data.est['sifgpp'].spatial
        sif_clim    = get_mean(-sif*(0.01/30)+nbp, all_periods)
        sif         = get_mean(-sif * (0.01 / 30)+nbp, spatial_period)
        trendy      = data.trendy['gpp'].spatial
    ...
    nirv = data.est['nirvgpp'].spatial
    nirv = get_season_mean(-nirv * 0.001+nbp, spatial_period, season)
    sif = data.est['sifgpp'].spatial
    sif = get_season_mean(-sif * (0.01 / 30)+nbp, spatial_period, season)
    trendy = data.trendy['gpp'].spatial
    ...
    if ano:
        ...
        nirv        = data.est['nirvgpp'].spatial
        nirv_clim   = get_season_mean(-nirv * 0.001+nbp, all_periods, season)
        nirv        = get_season_mean(-nirv * 0.001+nbp, spatial_period, season)
        sif         = data.est['sifgpp'].spatial
        sif_clim    = get_season_mean(-sif * (0.01 / 30)+nbp, all_periods, season)
        sif         = get_season_mean(-sif * (0.01 / 30)+nbp, spatial_period, season)
        trendy      = data.trendy['gpp'].spatial
