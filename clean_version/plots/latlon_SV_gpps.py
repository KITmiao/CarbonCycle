...
def plts(value, obj, cmap, range):
    ...
    return ax1, cbar
print(__name__)
if __name__.split('.')[-1] == 'latlon_SV_gpps':
    from settings import gpp_var
    ...
    if gpp_var == 'nirvgpp':
        variable = data.est[gpp_var].spatial['GPP'].sel(time=slice(spatial_period['start'], spatial_period['end']))
    elif gpp_var == 'sifgpp':
        variable = data.est[gpp_var].spatial['GOSIF'].sel(time=slice(spatial_period['start'], spatial_period['end']))
    variable = uni_transform(variable, gpp_var)
    std       = variable.groupby("time.year").std(dim='time')
    sv       = std.mean(dim='year')
    sv       = sa.zscore(sv)
    plot1, cbar1 = plts(sv, gpp_var, 'RdYlBu_r', [-3,3])
    cbar1.set_label('GPP seasonal variability')
