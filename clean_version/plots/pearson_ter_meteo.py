...
print(__name__)
if __name__.split('.')[-1] == 'pearson_ter_meteo':
    ...
    map, p_nirv = pearson_map(p_nirv, nbp-sif['GOSIF'], rain)
    ...
    map, t_nirv = pearson_map(t_nirv, nbp-sif['GOSIF'], temp)
    ...
    fig, p_nirv = LatlonPlot().create_map(region)
    map, p_nirv = pearson_map(p_nirv, nbp-nirv['GPP'], rain)
    LatlonPlot().corelation_cbar(fig, p_nirv, map)
    LatlonPlot().elev_spatial(p_nirv)
    ...
    fig, t_nirv = LatlonPlot().create_map(region)
    map, t_nirv = pearson_map(t_nirv, nbp-nirv['GPP'], temp)
    LatlonPlot().corelation_cbar(fig, t_nirv, map)
    LatlonPlot().elev_spatial(t_nirv)
