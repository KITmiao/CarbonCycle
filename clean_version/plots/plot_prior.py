...
print(__name__)
if __name__.split('.')[-1] == 'plot_prior':
    ...
    ax = VisualLines(ax, time).tm5_4dvar_prior(pris).ax
    ax = VisualLines(ax, time).tm5_4dvar_insitu(ins).ax
    ax = VisualLines(ax, time).inv_sat(gos, True).ax
    plt.legend(ncol=3)
    plt.show()

    fig, ax = plt.subplots(figsize=(10, 3))
    ax = MonthlyPlot(ax).co2_flux().ax
    ax = VisualLines(ax, time).cams_prior(pris).ax
    ax = VisualLines(ax, time).cams(ins).ax
    ax = VisualLines(ax, time).inv_sat(gos).ax
    plt.legend(ncol=3)
    plt.show()

    fig, ax = plt.subplots(figsize=(10, 3))
    ax = MonthlyPlot(ax).co2_flux().ax
    ax = VisualLines(ax, time).ct_cms_prior(pris).ax
    ...
    fig, ax = plt.subplots(figsize=(10, 3))
    ax = MonthlyPlot(ax).co2_flux().ax
    ax = VisualLines(ax, time).ct_4p1s_prior(pris).ax
    ax = VisualLines(ax, time).ct(ins).ax
    ax = VisualLines(ax, time).inv_sat(gos).ax
    plt.legend(ncol=3)
    plt.show()
